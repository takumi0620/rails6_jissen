...
describe "管理者による職員管理" do
  ...
  describe "新規登録" do
    # attributes_forはfactory botのメソッド
    # factories/staff_member.rbで定義した:staff_memberと同じ構造のハッシュが作れる
    let(:params_hash) { attributes_for(:staff_member) }

    example "職員一覧ページにリダイレクト" do
      # admin_staff_members_urlはroutes.rbで定義されたヘルパーメソッド
      post admin_staff_members_url, params: { staff_member: params_hash }
      expect(response).to redirect_to(admin_staff_members_url)
    end

    example "例外ActionContoller:PerameterMissing発生" do
